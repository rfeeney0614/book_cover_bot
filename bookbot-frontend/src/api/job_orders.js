export async function fetchJobOrders(params = {}) {
  ...
  const url = `/api/job_orders.json${qs ? `?${qs}` : ''}`;
...
export async function fetchJobOrder(id) {
  const res = await fetch(`/api/job_orders/${id}.json`, { headers: { Accept: 'application/json' } });
  if (!res.ok) {
    const text = await res.text().catch(() => '');
...
export async function createJobOrder(payload) {
  const res = await fetch(`/api/job_orders.json`, {
    method: 'POST',
...
export async function deleteJobOrder(id) {
  const res = await fetch(`/api/job_orders/${id}.json`, { method: 'DELETE' });
  if (!res.ok) {
    const text = await res.text().catch(() => '');
